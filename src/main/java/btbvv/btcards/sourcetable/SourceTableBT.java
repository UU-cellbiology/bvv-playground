 ...
 */
public class SourceTableBT extends JTable
{
	private final SourceTableModelBT model;
	...
	public SourceTableBT( final ViewerState state, final ConverterSetups converterSetups )
	{
		this( state, converterSetups, new InputTriggerConfig() );
	}

	public SourceTableBT( final ViewerState state, final ConverterSetups converterSetups, final InputTriggerConfig inputTriggerConfig )
	{
		this.state = state;
		converters = converterSetups;
		model = new SourceTableModelBT( state, converterSetups );
		setModel( model );
		setTransferHandler( new SourceTableTransferHandler() );

		getColumnModel().getColumn( IS_CURRENT_COLUMN ).setCellRenderer( new RadioButtonRendererBT() );
		getColumnModel().getColumn( COLOR_COLUMN ).setCellRenderer( new ColorRendererBT() );
	...
	@Override
	public SourceTableModelBT getModel()
	{
		return model;
	}
	...
	static class SourceTableTransferHandler extends TransferHandler
	{
		...
		protected Transferable createTransferable( final JComponent c )
		{
			...
			if ( ! ( table.getModel() instanceof SourceTableModelBT ) )
				return null;

			final SourceTableModelBT model = ( SourceTableModelBT ) table.getModel();
